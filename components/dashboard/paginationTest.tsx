...
export default function PaginationTest({ currentPage, limit, total }: {
  currentPage: number;
  limit: number;
  total: number;
}) {
  ...
  return (
    <section className="mt-4">
      <div className="flex justify-between">
        {currentPage ? (
          <Link 
          href={`/dashboard/products-test?page=0`} 
        ...
        {currentPage ? (
          <Link
            href={`/dashboard/products-test?page=${previousPage}`}
            className="text-sky-500 flex items-center gap-1"
          >
        ...
        {currentPage < lastPage ? (
          <Link
            href={`/dashboard/products-test?page=${nextPage}`}
            className="text-sky-500 flex items-center gap-1"
          >
        ...
        {currentPage < lastPage ? (
          <Link
            href={`/dashboard/products-test?page=${lastPage}`}
            className="text-sky-500 flex items-center gap-1"
          >
