...
export default function OfficeHours() {
  ...
  if (displayDay.includes('Saturday') || displayDay.includes('Sunday')) {
    return (
      <p suppressHydrationWarning>{displayDay} {displayTime} <FontAwesomeIcon icon={faCircleR} className="text-yellow-500" /> gone for the weekend</p>
    );
  }

  return (
    <p suppressHydrationWarning>{displayDay} {displayTime} {timestamp > openHours && timestamp < closingHours ? <FontAwesomeIcon icon={faCircle} className="text-green-500" /> : <FontAwesomeIcon icon={faCircleR} className="text-yellow-500" />} {timestamp > openHours && timestamp < closingHours ? "available" : "gone for the day"}</p>
  );
}
